...
def _calculate_average_daily_hours(
    ...
    while current_date <= end_date:
        ...
        # Attendance may have paid regular hours even when one timestamp is missing
        # (e.g., manual edits). Those days should count as wage-earning days.
        attendance_paid_hours_without_full_timestamps = (
            att and
            not worked_this_day and
        ...
        if worked_this_day:
            ...
            if regular > 0:
                ...
                if is_monday_friday or is_saturday or is_sunday:
                    days_worked += 1
                    days_eligible += 1
        elif attendance_paid_hours_without_full_timestamps:
            # Handle attendance records with payable hours but incomplete timestamps.
