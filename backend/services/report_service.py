...
class ReportService:
    ...
    def _get_sort_field_name(self, sort_by: SortField, data: List[Dict[str, Any]]) -> str:
        ...
        if sort_by == SortField.EMPLOYEE_NAME:
            ...
            # Map position field for reports with custom naming
            if 'position' in first_record:
                return 'position'
            elif 'current_position' in first_record:
    ...
    def _group_data(self, data: List[Dict[str, Any]], group_by: Optional[GroupByField], group_by_secondary: Optional[GroupByField] = None) -> List[GroupedReportData]:
        ...
        if group_by == GroupByField.EMPLOYEE and "employee_name" not in data[0] and "name" in data[0]:
            actual_field = "name"
        elif group_by == GroupByField.POSITION and "position" not in data[0] and "current_position" in data[0]:
            actual_field = "current_position"
    ...
    def generate_employee_personal_details_report(self, filters: EmployeeReportFilters) -> Tuple[Union[List[EmployeePersonalDetailsData], List[GroupedReportData]], ReportSummary]:
        """Generate employee personal details report"""
        try:
            # Get all employees using the existing repository function
            employees = search_employees(filters.search_term or "")
            ...
            report_data = []
            for employee in employees:
                report_data.append(EmployeePersonalDetailsData(
                    employee_id=employee.id,
            ...
            if filters.group_by:
                ...
                # No grouping, return sorted data
                sorted_data = [EmployeePersonalDetailsData(**record) for record in data_dicts]
            ...
            print(f"Error generating employee personal details report: {e}")
            return [], ReportSummary(
                total_records=0,
                total_pages=0,
                current_page=1,
    ...
    def generate_employee_basic_profile_report(self, filters: EmployeeReportFilters) -> Tuple[Union[List[EmployeeBasicProfileData], List[GroupedReportData]], ReportSummary]:
        ...
        try:
            ...
            companies = get_all_companies()
            company_lookup = {company.id: company.legal_name for company in companies}
            ...
            for employee in employees:
                ...
                current_employment = None
                for emp_record in employment_history:
                    if emp_record.end_date is None:
                        current_employment = emp_record
                        break

                if filters.position:
                    current_position = current_employment.position if current_employment else ""
                    if filters.position.lower() not in str(current_position).lower():
                        continue

                company_name = None
                if current_employment:
                    company_name = company_lookup.get(current_employment.company_id)

                report_data.append(EmployeeBasicProfileData(
                    name=employee.full_name,
                    company_name=company_name,
                    current_position=current_employment.position if current_employment else None,
                    age=self._calculate_age(getattr(employee, "dob", None))
                ))
            ...
            if filters.sort_by:
                ...
                data_dicts.sort(key=lambda x: x.get("name", "").lower())
            ...
            sorted_data = [EmployeeBasicProfileData(**record) for record in data_dicts]
            summary = ReportSummary(
                total_records=len(sorted_data),
                total_pages=1,
                current_page=1,
            ...
            print(f"Error generating employee basic profile report: {e}")
            return [], ReportSummary(
                total_records=0,
                total_pages=0,
                current_page=1,
