...
@router.get("/employee-personal-details", response_model=EmployeePersonalDetailsReportResponse)
async def get_employee_personal_details_report(
    # Common filters
    company_id: Optional[str] = Query(None, description="Filter by company ID"),
    department: Optional[str] = Query(None, description="Filter by department"),
    employee_status: Optional[str] = Query("Active & Probation", description="Filter by employee status"),
    ...
    """Generate employee personal details report showing Last name, First name, SIN, Address, Email, and Phone"""
    try:
        filters = EmployeeReportFilters(
            company_id=company_id,
            department=department,
        ...
        data, summary = report_service.generate_employee_personal_details_report(filters)
        
        # Check if data is grouped
        is_grouped = hasattr(data[0], 'group_key') if data else False
        
        return EmployeePersonalDetailsReportResponse(
            success=True,
            data=data,
            summary=summary,
            is_grouped=is_grouped
        ...
        raise HTTPException(status_code=500, detail=f"Error generating employee personal details report: {str(e)}")

@router.get("/employee-basic-profile", response_model=EmployeeBasicProfileReportResponse)
async def get_employee_basic_profile_report(
    # Common filters
    company_id: Optional[str] = Query(None, description="Filter by company ID"),
    department: Optional[str] = Query(None, description="Filter by department"),
    employee_status: Optional[str] = Query("Active & Probation", description="Filter by employee status"),
    ...
    # Employee-specific filters
    include_inactive: bool = Query(False, description="Include inactive employees"),
    position: Optional[str] = Query(None, description="Filter by current position"),
    hire_date_from: Optional[date] = Query(None, description="Filter by hire date from"),
    hire_date_to: Optional[date] = Query(None, description="Filter by hire date to"),
    ...
    """Generate employee basic profile report with Name, Company, Current Position, and Age"""
    try:
        filters = EmployeeReportFilters(
            company_id=company_id,
            department=department,
        ...
        data, summary = report_service.generate_employee_basic_profile_report(filters)
        is_grouped = hasattr(data[0], 'group_key') if data else False

        return EmployeeBasicProfileReportResponse(
            success=True,
            data=data,
            summary=summary,
            is_grouped=is_grouped
        ...
        raise HTTPException(status_code=500, detail=f"Error generating employee basic profile report: {str(e)}")
...
async def export_report(
    ...
    try:
        ...
        elif report_type == ReportType.EMPLOYEE_PERSONAL_DETAILS.value:
            data, summary = report_service.generate_employee_personal_details_report(
                EmployeeReportFilters(**filters.dict())
            )
        elif report_type == ReportType.EMPLOYEE_BASIC_PROFILE.value:
            data, summary = report_service.generate_employee_basic_profile_report(
                EmployeeReportFilters(**filters.dict())
            )
...
async def get_available_report_types(
    ...
    try:
        report_types = [
            ...
            {
                "type": ReportType.EMPLOYEE_PERSONAL_DETAILS.value,
                "name": "Employee Personal Details",
                "description": "Personal details report showing Last name, First name, SIN, Address, Email, and Phone"
            },
            {
                "type": ReportType.EMPLOYEE_BASIC_PROFILE.value,
                "name": "Employee Basic Profile",
                "description": "Employee report showing name, company, current position, and age"
            },
            {
...
async def get_available_filters(
    ...
    try:
        ...
        filter_options = {
            ...
            ReportType.EMPLOYEE_PERSONAL_DETAILS.value: {
                "common_filters": [
                    {"name": "company_id", "type": "select", "label": "Company", "description": "Filter by company"},
                    {"name": "department", "type": "text", "label": "Department", "description": "Filter by department"},
                    {"name": "employee_status", "type": "select", "label": "Status", "description": "Employee status", "options": ["Active", "On Leave", "Terminated", "Probation", "Active & Probation", "All"]},
            ...
            ReportType.EMPLOYEE_BASIC_PROFILE.value: {
                "common_filters": [
                    {"name": "company_id", "type": "select", "label": "Company", "description": "Filter by company"},
                    {"name": "department", "type": "text", "label": "Department", "description": "Filter by department"},
                    {"name": "employee_status", "type": "select", "label": "Status", "description": "Employee status", "options": ["Active", "On Leave", "Terminated", "Probation", "Active & Probation", "All"]},
                ...
                "specific_filters": [
                    {"name": "include_inactive", "type": "boolean", "label": "Include Inactive", "description": "Include inactive employees"},
                    {"name": "position", "type": "text", "label": "Position", "description": "Filter by current position"},
                    {"name": "hire_date_from", "type": "date", "label": "Hire Date From", "description": "Filter by hire date range"},
                    {"name": "hire_date_to", "type": "date", "label": "Hire Date To", "description": "Filter by hire date range"}
                ]
            },
