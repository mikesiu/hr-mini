...
async def delete_employee_by_id(
    ...
    """Delete an employee and return post-delete reference counts for consistency checks."""
    if not employee_exists(employee_id):
        raise HTTPException(status_code=404, detail="Employee not found")

    username = current_user.get("username") if isinstance(current_user, dict) else None
    success, report = delete_employee(employee_id, performed_by=username)
    if not success:
        if report and report.get("error") == "integrity_error":
            raise HTTPException(
                status_code=400,
                detail=(
                    "Cannot delete employee: related records still reference this employee "
                    "(database foreign keys may need migration). "
                    f"Details: {report.get('message', '')}"
                ),
            )
